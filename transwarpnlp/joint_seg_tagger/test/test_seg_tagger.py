...
def joint_test(train_dir, test_file, output_file):
    emb_path = None
    ng_emb_path = None

    model_path = train_dir + '/trained_model'
    weight_path = train_dir + '/trained_model_weights'
    ...
    chars, tags, grams = data_transform.read_vocab_tag(train_dir, ngram)
    ...
    new_chars = data_transform.get_new_chars(test_file, char2idx)
    ...
    test_x, test_y, test_max_slen_c, test_max_slen_w, test_max_wlen = \
        data_transform.get_input_vec(test_file, char2idx, tag2idx, tag_scheme=tag_scheme)
    ...
    if ngram > 1:
        ...
        new_grams = data_transform.get_new_grams(test_file, gram2idx)
        test_gram = data_transform.get_gram_vec(test_file, gram2idx)
        test_x += test_gram
...
def joint_predict(train_dir, predict_file, output_file):
    emb_path = None
    ng_emb_path = None
    ...
    new_chars = data_transform.get_new_chars(predict_file, char2idx, type='raw')
    char2idx, idx2char = data_transform.update_char_dict(char2idx, new_chars)

    if not config.tag_large:
        raw_x, raw_len = data_transform.get_input_vec_raw(predict_file, char2idx)
        print('Numbers of sentences: %d.' % len(raw_x[0]))
        max_step = raw_len
    else:
        max_step = data_transform.get_maxstep(predict_file)

    print('Longest sentence is %d. ' % max_step)
    if ngram > 1:
        gram2idx = data_transform.get_ngram_dic(grams)
        if not config.tag_large:
            raw_gram = data_transform.get_gram_vec(predict_file, gram2idx, is_raw=True)
            raw_x += raw_gram
    if not config.tag_large:
    ...
    with main_graph.as_default():
        ...
        with tf.device("/cpu:0"):
            ...
            with codecs.open(predict_file, 'r', encoding='utf-8') as l_file:
                lines = []
                for line in l_file:
                    ...
                    if len(lines) >= config.large_size:
                        ...
                        out = model.tag(sess=sess, r_x=raw_x, idx2tag=idx2tag, idx2char=idx2char,
                                        ...
                                        batch_size=config.tag_batch, large_file=True)
