...
def train_cnn_classfier(train_path):
    ...
    with tf.Graph().as_default(), tf.Session().as_default() as sess:
        ...
        batch_x_test, batch_y_test = rawdata.get_test_batch(test_revs, word_idx_map)

        for i in range(config.num_epoch):
            print("Start epoch:" + str(i))
            cnn_model.run(sess, model, train_revs, n_batches, word_idx_map)
            test_accuracy = model.accuracy.eval(feed_dict={model.x_in: batch_x_test, model.y_in: batch_y_test, model.keep_prob: 1.0})
